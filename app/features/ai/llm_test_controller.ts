...
import { embedTextValidator, chatValidator } from "#features/ai/llm_test_validator";
...
async function ensureCollection() {
  ...
  const exists = (collections?.result?.collections ?? []).some((c: any) => c.name === name);
  if (exists) return;
...
export default class LlmTestController {
  ...
  async embedText({ request, response }: HttpContext) {
    try {
      const { content, user, date } = await request.validateUsing(embedTextValidator);
  ...
  async chat({ request, response }: HttpContext) {
    try {
      ...
      const searchRes: any = await qdrantRequest(`/collections/chunks_test/points/search`, {
        method: "POST",
