...
export async function gotoJournal(page: Page) {
  ...
  if (page.url().includes('/login')) {
    throw new Error('Journal page is protected. Please ensure the test user is logged in or provide a login helper.');
  }
...
export async function selectRange(page: Page, from: number, to: number) {
  await page.evaluate((params: {from: number, to: number}) => {
    const editorElement = document.querySelector('.cm-editor') as any;
    const view = editorElement?.cmView;
    if (view) {
...
export async function getLineText(page: Page, lineNumber: number): Promise<string> {
...
export async function hasHiddenMarkers(page: Page, lineNumber: number): Promise<boolean> {
  return await page.evaluate((line: number) => {
